...
export function CrudTable<T extends { id: string }>({
  ...
  const getStatusConfig = (
    level: EstadosDefinidos = "neutral",
    customClass: string = ""
  ) => {
    const base =
      "border font-medium text-xs px-2.5 py-0.5 rounded-full flex items-center gap-1.5 shadow-sm transition-colors";

    const styles: Record<EstadosDefinidos, string> = {
      positive:
        "bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100", // Activo
      ...
      personalizado:
        "bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100", // Desconocido
    };
  ...
  // --- MOBILE VIEW ---
  if (!isDesktop) {
    // 1. Separamos las columnas por grupos
    const textRelevanteCols = columns.filter(
      (c) => c.level === TypeLevel.textRelevante
    );

    // 2. Columnas para el Grid (excluyendo coordenadas explícitamente)
    const gridColumns = columns.filter(
      (c) =>
        ![
          TypeLevel.id,
          TypeLevel.foto,
          TypeLevel.titulo,
          TypeLevel.subtitulo,
          TypeLevel.textRelevante,
          TypeLevel.coordenada,
        ].includes(c.level as string)
    );
      ...
      <div className="w-full space-y-4 animate-in fade-in duration-500">
          ...
          <div className="rounded-xl border border-[var(--colorSecondary)] bg-white py-12 text-center shadow-sm">
            <div className="flex flex-col items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-[var(--bgGeneral)] flex items-center justify-center border border-[var(--colorSecondary)]">
                {Icon && (
                  <Icon className="w-5 h-5 text-[var(--textColor)]/50" />
                )}
              </div>
              <div>
                ...
                <p className="text-xs text-[var(--textColor)]/60 mt-1">
                  {searchTerm
                    ...
                    : "Crea un registro para comenzar"}
                </p>
              </div>
            </div>
          </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                  <div className="flex items-center gap-2">
                    ...
                    <h3
                      className={`${
                        firstTitle?.classNameTitle || "font-bold text-slate-800"
                      } truncate text-sm`}
                    >
                  ...
                  <p
                    className={`${
                      firstSubtitle?.classNameText || "text-slate-500"
                    } text-xs truncate flex items-center gap-1.5`}
                  >
                    {firstSubtitle?.Icon && (
                      <firstSubtitle.Icon className="w-3 h-3 opacity-70" />
                    )}
                ...
                <div
                  className={`shrink-0 text-[var(--textColor)]/40 transition-transform duration-300 ${
                    expandedId === item.id
                      ? "rotate-180 text-[var(--colorPrimary)]"
                      : ""
                  }`}
                >
                  ...
                  <div className="p-3.5 border-t border-[var(--colorSecondary)]/50">
                      ...
                      <div className="flex flex-wrap gap-2 pb-3 mb-3 border-b border-[var(--colorSecondary)]/60">
                        {textRelevanteCols.map((col: any) => {
                          ...
                          return (
                            <div
                              key={col.key}
                              className={getStatusConfig(value, customClass)}
                            >
                              {col.Icon ? (
                                <col.Icon className="w-3 h-3 opacity-70" />
                              ) : (
                                <StatusDot level={value} />
                              )}
                              <span className="opacity-70 mr-1 ml-0.5 font-normal">
                                {col.label}:
                              </span>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {gridColumns.map((col) => {
                        ...
                        if (col.level === TypeLevel.tags) {
                          ...
                          return (
                            <div
                              ...
                              className="col-span-2 bg-white p-3 rounded-lg border border-[var(--colorSecondary)] flex flex-col shadow-sm"
                            >
                        ...
                        // DEFAULT (Texto Normal y Fechas)
                        return (
                          <div
                            key={col.key as string}
                            className="col-span-1 bg-white p-3 rounded-lg border border-[var(--colorSecondary)] flex flex-col justify-center min-h-[70px] shadow-sm hover:border-slate-300 transition-colors"
                          >
                    ...
                    <div className="flex items-center gap-2 mt-4 pt-3 border-t border-[var(--colorSecondary)]/50">
                      {/* Botón de Mapa eliminado */}
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => onEdit?.(item)}
                        className="flex-1 bg-white border-[var(--colorSecondary)] text-[var(--textColor)]/80 hover:text-indigo-600 hover:bg-indigo-50 h-9"
                      >
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="outline"
                            size="icon"
                            className="h-9 w-9 bg-white border-[var(--colorSecondary)] text-[var(--textColor)]/70 shrink-0"
                          >
        ...
        <div className="flex items-center justify-between px-4 py-4 bg-white border border-[var(--colorSecondary)] rounded-xl shadow-sm mt-2">
          <div className="text-xs text-[var(--textColor)]/60 font-medium">
            <span className="text-[var(--textColor)] font-bold">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <SelectTrigger className="h-8 w-16 text-xs bg-slate-50 border-[var(--colorSecondary)]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <TableHeader>
              <TableRow className="bg-gradient-to-r from-[var(--colorPrimary)]/8 to-[var(--colorAcentuar)]/8 hover:bg-gradient-to-r hover:from-[var(--colorPrimary)]/12 hover:to-[var(--colorAcentuar)]/12 border-b-2 border-[var(--colorSecondary)]">
                {columns.map((col) => (
                  <TableHead
                    key={col.key as string}
                    className={col.classNameTitle}
                  >
                    {col.label}
                  </TableHead>
                ))}
                <TableHead className="text-right text-[var(--textColor)] font-bold text-sm">
                  Acciones
                </TableHead>
              </TableRow>
            </TableHeader>
                  ...
                  >
                    <div className="flex flex-col items-center gap-4">
                      <div className="w-12 h-12 rounded-full bg-[var(--bgGeneral)] flex items-center justify-center border-2 border-[var(--colorSecondary)]">
                        {Icon && (
                          <Icon className="w-6 h-6 text-[var(--textColor)]/40" />
                        )}
                      </div>
                      <div>
                  ...
                  >
                    {columns.map((col) => {
                      ...
                      if (col.level === TypeLevel.textRelevante) {
                        ...
                        // Eliminado TypeLevel.coordenada
                      } else if (col.level === TypeLevel.fecha) {
                        return (
                          <TableCell
                            key={col.key as string}
                            className={col.classNameText}
                          >
                            <div className="flex items-center gap-2">
                        ...
                        const imgUrl = (item as any)[col.key];
                        return (
                          <TableCell
                            key={col.key as string}
                            className={col.classNameText}
                                ...
                                <HoverCardTrigger asChild>
                                  <div className="h-9 w-9 rounded-full overflow-hidden border border-slate-200 cursor-pointer hover:ring-2 hover:ring-blue-400 transition-all shadow-sm">
                                    <img
                                      ...
                                      onError={(e: any) =>
                                        (e.target.src =
                                          "https://via.placeholder.com/32?text=NA")
                                      }
                                    />
                                  </div>
                                </HoverCardTrigger>
                                ...
                                >
                                  <div className="rounded-xl overflow-hidden border-4 border-white shadow-2xl">
                                    <img
                                      ...
                                      onError={(e: any) =>
                                        (e.target.src =
                                          "https://via.placeholder.com/200?text=NA")
                                      }
                                    />
                                  </div>
                                </HoverCardContent>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-1.5 flex-wrap max-w-[240px]">
                                    ...
                                    >
                                      <div className="flex flex-col gap-0.5 min-w-[140px] max-w-[200px] max-h-[180px] overflow-y-auto custom-scrollbar p-1">
                                        ...
                                        {values.slice(limit).map((val) => (
                                          <div
                                            key={val.value}
                                            className="px-2 py-1.5 rounded-md hover:bg-slate-100 text-xs text-slate-700 font-medium transition-colors cursor-default text-left"
                                          >
                              ...
                              {values.length === 0 && (
                                <span className="text-slate-300 text-xs font-light italic px-1 select-none">
                                  —
                                </span>
                              )}
                            </div>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent
                            align="end"
                            className="border-[var(--colorSecondary)]"
                          >
                            <DropdownMenuItem
                              onClick={() => onEdit?.(item)}
